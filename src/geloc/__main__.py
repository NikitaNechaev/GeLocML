...
# Нет необходимости добавлять src в путь, так как мы уже внутри пакета
# Но если запускаем напрямую как скрипт, то нужно добавить родительскую директорию
if __package__ is None or __package__ == '':
    # Когда файл запускается напрямую: python src/geloc/__main__.py
    sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from .core import run_geloc
from .utils import print_version, print_help
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='Примеры использования:\n'
               '  geloc --train --data data/raw/train.csv\n'
               '  geloc --predict --model model.pkl --input test.fasta\n'
               '  geloc --version\n'
               '  geloc --help'
    )
